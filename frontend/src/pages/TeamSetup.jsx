...
// 🆕 Add this API call here
async function clearAllQuestions() {
  const confirmClear = window.confirm("⚠️ Are you sure you want to delete ALL questions?");
  if (!confirmClear) return;

  const res = await fetch(`${import.meta.env.VITE_API_BASE || "https://church-quizzler.onrender.com"}/questions/clear`, {
    method: "DELETE",
  });

  if (!res.ok) {
    const err = await res.json().catch(() => ({}));
    alert("❌ Failed to clear questions: " + (err.detail || res.statusText));
    return;
  }
    ...
    <div className="w-full min-h-screen p-8 bg-gradient-to-br from-yellow-50 via-white to-purple-50 font-serif">
      ...
      <div className="flex flex-col lg:flex-row gap-6 mb-10 justify-center items-center">
        ...
        <input
          ...
          className="w-36 px-4 py-3 border-2 border-purple-200 rounded-full text-lg text-center focus:outline-none focus:ring-4 focus:ring-purple-300"
          placeholder="Timer (s)"
        />
        <button
          onClick={addTeam}
          className="px-10 py-3 bg-gradient-to-r from-purple-600 to-yellow-400 text-white text-lg rounded-full shadow-lg hover:scale-105 transition"
        >
          Add Team
        </button>
      </div>
      ...
      {teams.length === 0 ? (
        <p className="text-gray-500 mb-12 text-center">No teams yet.</p>
      ) : (
        <ul className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-12">
          {teams.map((t) => (
            <li
              ...
              style={{ backgroundColor: t.color || "#6A0DAD" }}
